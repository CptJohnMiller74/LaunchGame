...
public class playerController : MonoBehaviour {
    ...
    void Start () {
        ...
        Debug.Log("max boosts = " + upgradeManager.maxBoosts + "\n groundspeedreduction = " + upgradeManager.groundSpeedReduction);
        Debug.Log("maxboostsindex = " + upgradeManager.maxBoostsIndex);
    }
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.GetComponent<enemyController>() != null && !collision.GetComponent<enemyController>().getIsDead() && !isCollidingWithEnemy)
        {
            ...
            if (wasBoostingOnCollision)
            {
                ...
                {
                    killEnemy(enemy, 5, enemy.GetComponent<enemyController>().boostPoints);
                }
                enemiesToBeKilled.Clear();
            }
            else
            {
                killEnemy(collision.gameObject, 5, collision.gameObject.GetComponent<enemyController>().boostPoints);
            }
    ...
    public void setIsBoosting(bool b)
    {
        ...
        {
            hitCollider.size = new Vector2(normalColliderWidth, hitCollider.size.y);
        }
    }
